...
app.get('/usuario/buscarcpf/:cpf', async (req, res) => {
  ...
  if (query.length === 0)
    return res
      .status(400)
      .json({ mensagem: 'Nenhum usuário encontrado por este CPF!' });
  return res.status(200).json(query);
});
...
app.post('/usuario', async (req, res) => {
  ...
  const [query] = await conection.execute(
    'insert into usuario (nome, sobrenome, senha, cpf) values (?, ?, ?, ?)',
    [nome, sobrenome, senha, cpf],
  );
...
app.put('/usuario/:id', async (req, res) => {
  ...
  const { nome, sobrenome, senha, cpf } = req.body;
  const [query] = await conection.execute(
    'update usuario set nome = ?, sobrenome = ?, senha = ?, cpf = ? where id = ?',
    [nome, sobrenome, senha, cpf, id],
  );
  return res.json(query);
});
...
app.get('/categoriareceita/:id', async (req, res) => {
  ...
  if (query.length === 0)
    return res.status(400).json({
      mensagem: 'Nenhuma categoria-receita encontrado por este id no database!',
    });
  return res.status(200).json(query);
});
...
app.get('/categoriadespesa/:id', async (req, res) => {
  ...
  if (query.length === 0)
    return res.status(400).json({
      mensagem: 'Nenhuma categoria-despesa encontrado por este id no database!',
    });
  return res.status(200).json(query);
});
...
app.get(
  '/categoriadespesa/buscarcategoriadespesanome/:nome',
  async (req, res) => {
    const { nome } = req.params;
    const [query] = await conection.execute(
...
app.get(
  '/categoriadespesa/buscarcategoriadespesatipo/:tipo',
  async (req, res) => {
    const { tipo } = req.params;
    const [query] = await conection.execute(
...
app.put('/categoriadespesa/:id', async (req, res) => {
  ...
  const { nome, tipo, usuario_id } = req.body;
  const [query] = await conection.execute(
    'update categoria_despesa set nome = ?, tipo = ?, usuario_id = ? where id = ?',
    [nome, tipo, usuario_id],
  );
  return res.json(query);
});
...
app.get('/receitas', async (req, res) => {
  ...
  if (resultado.length === 0) {
    return res
      .status(200)
      .json({ mensagem: 'Nenhuma receita encontrada no database!' });
  }
  return res.status(200).json(resultado);
});
...
app.get('/receitas/:id', async (req, res) => {
  ...
  if (query.length === 0)
    return res.status(400).json({
      mensagem: 'Nenhuma receita encontrada por este id no database!',
    });
  return res.status(200).json(query);
});
...
app.delete('/receitas/:id', async (req, res) => {
  const { id } = req.params;
  const [query] = await conection.execute('DELETE FROM receitas WHERE id = ?', [
    id,
  ]);
  return res.json(query);
});
...
app.get('/despesas', async (req, res) => {
  ...
  if (resultado.length === 0) {
    return res
      .status(200)
      .json({ mensagem: 'Nenhuma despesa encontrada no database!' });
  }
  return res.status(200).json(resultado);
});
...
app.get('/despesas/:id', async (req, res) => {
  ...
  if (query.length === 0)
    return res.status(400).json({
      mensagem: 'Nenhuma despesa encontrada por este id no database!',
    });
  return res.status(200).json(query);
});
...
app.delete('/despesas/:id', async (req, res) => {
  const { id } = req.params;
  const [query] = await conection.execute('DELETE FROM despesas WHERE id = ?', [
    id,
  ]);
  return res.json(query);
});
